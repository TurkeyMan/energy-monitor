...
#        DFLAGS := $(DFLAGS) -mtriple=x86_64-unknown-linux-gnu
    else ifeq ($(PLATFORM),x86)
        DFLAGS := $(DFLAGS) -mtriple=i686-unknown-linux-gnu
    else ifeq ($(PLATFORM),arm64)
        DFLAGS := $(DFLAGS) -mtriple=aarch64-unknown-linux-gnu
    else ifeq ($(PLATFORM),arm32)
        DFLAGS := $(DFLAGS) -mtriple=arm-unknown-linux-eabihf -mcpu=cortex-a7
    else
