                ...
                if (el.value.length > 0)
                {
                    final switch (el.type)
                    {
                        ...
                        case ElementTemplate.Type.Map:
                            ...
                            // HACK: delete this, it's just rubbish
                            import urt.si;
                            ScaledUnit unit;
                            float scale;
                            ptrdiff_t taken = unit.parseUnit((*pReg).units[], scale);
                            if (taken == e.unit.length)
                            {
                                // we have a valid unit!!
                            }
//                            const(char)[] unit = (*pReg).units[];
...
//                            e.arrayLen = 0; // TODO: handle arrays?
                            ...
                            // TODO: init the value properly
                            // write zero with the proper type, and unit if specified...
