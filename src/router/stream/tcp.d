...
nothrow @nogc:
    ...
    final override CompletionStatus startup()
    {
        ...
        if (_socket == Socket.invalid)
        {
            ...
            if (!r)
            {
                debug writeError(type, " '", name, "' - create_socket() failed with error: ", r.get_SocketResult());
            ...
            {
                debug writeWarning(type, " '", name, "' - connect() failed with error: ", r.get_SocketResult());
                return CompletionStatus.Error;
            }
        }
        ...
        if (r.failed)
        {
            debug writeError(type, " '", name, "' - poll() failed with error: ", r.get_SocketResult);
            return CompletionStatus.Error;
        }
...
nothrow @nogc:
    ...
    final override CompletionStatus startup()
    {
        ...
        r = _serverSocket.set_socket_option(SocketOption.NonBlocking, true);
        if (r.failed)
        {
