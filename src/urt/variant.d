...
//        if (is(S : const(char)[]))
    ...
    double asDouble() const pure @property
    {
        ...
        if ((flags & Flags.DoubleFlag) != 0)
            r = value.d;
        else if ((flags & Flags.UintFlag) != 0)
            r = cast(double)cast(uint)value.ul;
        else if ((flags & Flags.IntFlag) != 0)
            r = cast(double)cast(int)value.l;
        else if ((flags & Flags.Uint64Flag) != 0)
            r = cast(double)value.ul;
        else
            r = cast(double)value.l;
        if ((flags & Flags.IsQuantity) != 0 && (count >> 24) != 0)
            assert(false, "TODO: scale by the unit scale...");
        return r;
    }
...
//            r.value = asFloat();
            ...
            r.value = cast(int)asInt();
        else if (is(T == uint))
            r.value = cast(uint)asUint();
        else if (is(T == long))
            r.value = cast(long)asLong();
        else if (is(T == ulong))
            r.value = cast(ulong)asUlong();
        else
